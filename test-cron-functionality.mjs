...
import { generateAISummaryWithRetry } from './services/filing/summaryGenerationService.ts';
...
async function testDatabaseAndEmailFunctionality() {
  ...
  try {
    ...
    if (cikMappings === 0) {
      ...
      // Add some sample CIK mappings for testing
      console.log('📝 Adding sample CIK mappings for testing...');
      const sampleMappings = [
        { cik: '0000320193', ticker: 'AAPL', companyName: 'Apple Inc.' },
        { cik: '0001318605', ticker: 'TSLA', companyName: 'Tesla, Inc.' },
        { cik: '0000789019', ticker: 'MSFT', companyName: 'Microsoft Corporation' },
        { cik: '0001652044', ticker: 'GOOGL', companyName: 'Alphabet Inc.' },
      ];
      
      for (const mapping of sampleMappings) {
        await prisma.cikMapping.upsert({
          where: { cik: mapping.cik },
          update: mapping,
          create: mapping
        });
      }
      
      const newCount = await prisma.cikMapping.count();
      console.log(`✅ Added CIK mappings, total now: ${newCount}`);
    }
    ...
    try {
      const emailModule = await import('./lib/email/summary-service.ts');
      console.log('✅ Email service module loaded successfully');
      console.log('✅ sendFilingSummaryEmail function available');
    } catch (error) {
      console.log('⚠️  Email service import issue:', error.message);
    }
