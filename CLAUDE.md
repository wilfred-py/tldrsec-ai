...
15. **Don't use `{ not: null }` or `{ NOT: { field: null } }` on required Prisma fields** - If a field is `String` (not `String?`), it can never be null. Prisma rejects null filters on required fields. Check `prisma/schema.prisma` before adding null-guard where clauses.
16. **SVG elements need explicit pixel dimensions, not CSS classes** - `w-screen h-screen` or `100%` on SVG elements doesn't reliably set the coordinate system. Use `width={px} height={px} viewBox="0 0 {px} {px}"` with values tracked in React state via `window.innerWidth/innerHeight`
17. **Verify correct dev server port when using worktrees** - Worktree dev servers auto-assign the next available port (e.g., 3001 if 3000 is taken by main repo). Always check `lsof -Pi :3000 -sTCP:LISTEN` and `lsof -Pi :3001 -sTCP:LISTEN` to confirm which server has worktree code. Testing on the wrong port shows old/main branch behavior
...
| Handler test w/ mocks | `__tests__/cron/handlers/summarize-cached-handler-validation.test.ts` |
