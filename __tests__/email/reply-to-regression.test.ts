 ...
 *
 *   2. The welcome-email path (now `queueWelcomeEmail` private to
 *      `app/(auth)/onboarding/actions.ts` since the shallow
 *      `lib/email/welcome-service.ts` module was inlined into its only caller)
 *      sets replyTo EXPLICITLY on the EmailMessage, so a future config
 *      rollback can't silently re-break replies. Belt-and-braces.
 *
