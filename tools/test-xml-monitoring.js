 ...
 * 
 * Usage:
 *   node tools/test-xml-monitoring.js [filingId]
 */

import { getFilingById } from '../services/filing/getFilingById.js';
 ...
 */
async function testXmlMonitoring(filingId) {
  try {
    ...
    // Parse URL attempts
    const urlAttempts = JSON.parse(fetchAttempt.urlAttempts || '[]');
    
    // Find the first successful attempt with XML content
    const xmlAttempt = urlAttempts.find((a) => a.success && a.xmlSummary);
