            ...
            <div className="text-xs text-muted-foreground space-y-1">
              {user.subscriptionTier === 'FREE' && (
                <div>
                  ...
                  {` Updates every ${TIER_CONFIG.PROFESSIONAL.frequency}`}
                </div>
              )}
              {(user.subscriptionTier === 'FREE' || user.subscriptionTier === 'PROFESSIONAL') && (
                <div>
                  ...
                  {` Updates every ${TIER_CONFIG.ENTERPRISE.frequency}`}
                </div>
              )}
              {/* Always show Institution tier upgrade option (we're already inside !INSTITUTION block) */}
              <div>
                <span className={TIER_CONFIG.INSTITUTION.textColor}>Institution:</span>
                {` Updates every ${TIER_CONFIG.INSTITUTION.frequency}`}
              </div>
            </div>
          </div>
        )}
