...
### Added
- **Materiality signal on 10-K / 10-Q summary emails (PR1 of 3, behind `enable_earnings_mini_deep_dive` flag).** Each annual/quarterly summary now classifies overall materiality as `high | medium | low | noise` with a 40-400 char rationale, surfaced as a colored badge in the email header (amber/indigo/slate). The badge reuses the existing `FormPlusMaterialityBadgeRow` `signal` slot — no double-render — and `noise` collapses cleanly back to the form-type label ("ANNUAL REPORT" / "QUARTERLY REPORT"). One-click `mailto:materiality-feedback@tldrsec.com` link under the badge so a reader can flag a wrong call (encodes ticker + form + accession in the subject for triage). Rubric is **symmetric** — large beats are as material as large misses — to avoid the asymmetric-classifier failure mode that would have systematically underweighted positive surprises.
- **Materiality calibration harness** at `scripts/materiality-calibration/`. Reproducible 3-step pipeline: `sample-filings.ts` pulls a stratified 30-filing set (~15 × 10-K, ~15 × 10-Q, ≤2 per ticker) from `pipeline.FilingContentCache` joined to `app.CikMapping`; human/Opus labels are checked into `labels.csv`; `run-calibration.ts` runs the production rubric against the same filings with the production model (grok-4.3) and emits a confusion matrix plus pass/fail at the 75% accuracy gate. **Production rubric passes at 76.7% (23/30)** on the v6 labeled set after six iterations: 10% → 50% → 40% → 40% → 73.3% → **76.7%**. Total cost across all six runs: $1.92. The harness lives alongside the rubric (`lib/ai/calibration/materiality-rubric.ts`) so any future rubric edit can be re-validated against the same labels in <2 minutes.
- **44 tests across 3 new suites.** `__tests__/email/materiality.test.ts` (18) covers signal extraction with malformed JSON, missing fields, unknown enum values; `__tests__/email/materiality-badge-rendering.test.tsx` (11) asserts the badge renders correct colors per score and collapses to the form-type label on `noise`; `__tests__/ai/prompts/materiality-flag-gate.test.ts` (15) confirms the schema property AND the rubric guidance block are stripped from the prompt when the feature flag is off, so off-flag filings can't accidentally produce a materiality field. All 93 PR1-related tests green.

### Changed
- **Prior-period financial values now render side-by-side in scorecards.** Each row in the financial-highlights table shows `[light-grey prior] → [current]` with a colored pill chip for the percent delta, instead of just the current value plus a small ↓/↑ arrow. Applied to both the 10-K and 10-Q templates. Pills use the same red/green/gray semantics already established in `EmailColors.semantic.pill*Bg`. Required adding an optional `priorValue` field to `FINANCIAL_HIGHLIGHT_ITEM` in the shared schema; the 10-Q inline `financialHighlights` schema gained the same field.
- **10-Q "Why it matters" block uses pill chips, never inline arrows.** The string-built prose ("Revenue grew ↑12.3% to $X.X B") was hard to read in Gmail and especially noisy when multiple metrics were chained. Refactored `whyItMatters` from a `string` to a `React.ReactNode` so embedded `<PillDelta>` components render inline ("Revenue grew [+12.3%] to $X.X B"). 10-K already used pills — this brings 10-Q to parity.
- **10-K segment mix renders as a horizontal bar chart, not prose.** The "Data Center: $115B / Compute: $50B / Networking: $20B" sentence in `storyParts` is replaced with a proportional table-cell bar chart (Gmail-safe: nested `<table>` with width percentages, no SVG, no flexbox). The text version is removed so the same segment data doesn't double-render.

### Notes
- **No production behavior change without the PostHog flag.** `enable_earnings_mini_deep_dive` is currently off everywhere. Filings processed off-flag get exactly the same prompt as before (the materiality property and rubric guidance are stripped in `generateFilingPrompt`), and the email renders the form-type label like it always did (because `extractMaterialitySignal` returns the noise default when the field is absent). This means PR2/PR3 can ship without forcing a regenerate of historical summaries.
- **Why a single-layer gate.** Earlier drafts gated at three layers (schema, prompt, render). Collapsed to one: if the field never enters the JSON, the render path's noise default takes over for free. Three-layer gating creates three places where they can drift out of sync; the flag-gate test suite asserts the schema-strip is the load-bearing contract.
