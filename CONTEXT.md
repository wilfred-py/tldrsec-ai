...
### Filing Prompt
The system + user prompt (and JSON output schema) handed to the model to turn a [Filing] into a [Summary]. The live deep module is `lib/ai/prompts/unified-prompts.ts` (`generateFilingPrompt` + `getSchemaForFormType`), consumed by the Phase-4 summarization pipeline (`lib/ai/summarize.ts`) and `filing-analyzer.ts`; an older template-string path (`filing-prompts.ts` + `prompt-templates.ts`) still serves `chunk-processor.ts`. A third, parallel prompt vocabulary — the `PromptTemplate` base class plus per-form `Form10KPrompt`/`Form10QPrompt`/`Form8KPrompt`/`FormForm4Prompt`/`GenericPrompt` subclasses (`prompt-template.ts`, `form-10k.ts`, `form-10q.ts`, `form-8k.ts`, `form-4.ts`, `generic.ts`) — was deleted: it had no production caller and only its own two test files instantiated it, so its "journalist tone" assertions gave false confidence while never exercising the real path. Not to be confused with the `PromptTemplate` *interface* in `prompt-types.ts`, a data shape the surviving template-string path still uses.
