...
import { appRouterAsyncHandler, createInternalError, ApiError, ErrorCode } from '@/lib/error-handling';
    ...
    try {
        ...
        switch (job.jobType) {
            ...
            case 'CHECK_FORM4_FILINGS':
                ...
                break;
            default:
                throw new ApiError(ErrorCode.BAD_REQUEST, `Unsupported job type: ${job.jobType}`, { jobId });
        }
        ...
        if (error instanceof Error) {
            ...
            try {
                ...
                if (job.attempts >= MAX_ATTEMPTS - 1) {
                    ...
                    await DeadLetterQueueService.addToDeadLetterQueue(jobId, job.jobType, job.payload, error, job.attempts + 1);
...
export const GET = appRouterAsyncHandler(async (request) => {
    ...
    const specificJobType = typeParam ? typeParam : undefined;
    ...
    try {
        ...
        // Process each job
        const results = await Promise.all(jobs.map((job) => processJob(job.id)));
