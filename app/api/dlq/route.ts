...
export const GET = appRouterAsyncHandler(async (request: Request) => {
  ...
  const entries = await DeadLetterQueueService.getDeadLetterEntries(
    limit,
    offset,
    includeReprocessed
  );
 ...
 * }
 */
export const POST = appRouterAsyncHandler(async (request: Request) => {
  const startTime = Date.now();
  ...
  // Requeue the item
  const newJobId = await DeadLetterQueueService.requeueDeadLetterEntry(
    body.id,
    addJobFunction
  );
 ...
 */
export const DELETE = appRouterAsyncHandler(async (request: Request) => {
  const startTime = Date.now();
