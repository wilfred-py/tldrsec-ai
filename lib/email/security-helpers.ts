...
export function maskUserIdForLogging(userId: string): string {
  ...
  try {
    ...
    if (userId.length <= 4) {
      return userId; // Very short IDs preserved
 ...
 */
export function maskEmailContentForLogging(content: string, maxLength: number = 50): string {
 ...
 */
export function createGDPRCompliantLogData(originalData: Record<string, unknown>): Record<string, unknown> {
    ...
    switch (key.toLowerCase()) {
      ...
      case 'metadata':
        if (typeof value === 'object' && value !== null) {
          sanitized[key] = createGDPRCompliantLogData(value);
        } else {
          sanitized[key] = value;
        }
        break;
...
/**
 * Distributed lock implementation for email queue processing
 * Prevents race conditions in email processing
 */
export class EmailQueueLock {
  private static activeLocks = new Map<string, { timestamp: number; processId: string }>();
  private static readonly LOCK_TIMEOUT = 5 * 60 * 1000; // 5 minutes
  
  /**
   * Acquire a lock for email queue processing
   * @param lockKey Unique key for the lock
   * @param processId Unique identifier for the process requesting the lock
   * @returns True if lock was acquired, false otherwise
   */
  static acquireLock(lockKey: string, processId: string): boolean {
    const now = Date.now();
    
    // Clean up expired locks
    for (const [key, lock] of EmailQueueLock.activeLocks.entries()) {
      if (now - lock.timestamp > EmailQueueLock.LOCK_TIMEOUT) {
        EmailQueueLock.activeLocks.delete(key);
      }
    }
    
    // Check if lock is already held
    const existingLock = EmailQueueLock.activeLocks.get(lockKey);
    if (existingLock && existingLock.processId !== processId) {
      return false; // Lock is held by another process
    }
    
    // Acquire the lock
    EmailQueueLock.activeLocks.set(lockKey, {
      timestamp: now,
      processId
    });
   ...
   * Release a lock
   * @param lockKey Unique key for the lock
   * @param processId Unique identifier for the process releasing the lock
   */
  static releaseLock(lockKey: string, processId: string): void {
    const existingLock = EmailQueueLock.activeLocks.get(lockKey);
    if (existingLock && existingLock.processId === processId) {
      EmailQueueLock.activeLocks.delete(lockKey);
    }
  }
  ...
  static isLocked(lockKey: string): boolean {
    const lock = EmailQueueLock.activeLocks.get(lockKey);
   ...
   */
  static getLockStatus(): { [key: string]: { processId: string; age: number } } {
    const now = Date.now();
    const status: { [key: string]: { processId: string; age: number } } = {};
