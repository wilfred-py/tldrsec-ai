...
import { 
  TickerResolutionOptions, 
  TickerResolutionResult,
  SECCompanyData,
  CikMappingData
} from './types';

// Cache for ticker-to-CIK mappings to reduce database calls
type CacheEntry = {
  cik: string;
  companyName: string;
  timestamp: number;
...
export class TickerResolver {
  ...
  private secClient: SECDataClient;
  private cache: Map<string, CacheEntry> = new Map();
  private cacheTimeoutMs: number = 24 * 60 * 60 * 1000; // 24 hours
  private cacheMaxSize: number = 1000; // Maximum cache size

  constructor(options: {
    prisma?: PrismaClient;
    secClient?: SECDataClient;
  ...
  async resolveTicker(
    ...
    const result: TickerResolutionResult = {
      success: false,
      ticker,
      normalizedTicker
    };
    ...
    try {
      ...
      return {
        ...result,
        success: false,
        error: `Ticker ${normalizedTicker} not found in database or SEC API`
      };
    } catch (error) {
   ...
   */
  private async fetchAndStoreCikInformation(
    ticker: string,
    options: TickerResolutionOptions
  ): Promise<TickerResolutionResult> {
    try {
      ...
      await this.recordFailedLookup(ticker, (error as Error).message);
      
      return {
        success: false,
        ticker,
        normalizedTicker: ticker,
        error: `Failed to fetch CIK information from SEC: ${(error as Error).message}`
      };
    }
  }
   ...
   */
  private async recordFailedLookup(ticker: string, errorMessage: string): Promise<void> {
    try {
   ...
   */
  private processCompanyData(data: SECCompanyData): Omit<CikMappingData, 'id' | 'lastUpdated' | 'fetchAttempts'> {
    // Format CIK with leading zeros
    const formattedCik = data.cik.padStart(10, '0');
    
    // Process former names into the format we need
    const formerNames: Record<string, { from: string; to: string }> = {};
   ...
   */
  private async createCikMapping(data: Omit<CikMappingData, 'id' | 'lastUpdated' | 'fetchAttempts'>): Promise<CikMappingData> {
    return await this.prisma.cikMapping.create({
      data: {
        ...data,
        fetchAttempts: 1
      }
    }) as unknown as CikMappingData;
  }
  
  /**
   * Find a ticker in the database
   */
  private async findInDatabase(ticker: string): Promise<CikMappingData | null> {
    try {
      ...
      return mapping as unknown as CikMappingData | null;
    } catch (error) {
   ...
   */
  private checkCache(ticker: string): CacheEntry | null {
   ...
   */
  private addToCache(ticker: string, entry: CacheEntry): void {
   ...
   */
  public normalizeTicker(ticker: string): string {
   ...
   */
  async refreshTickerInfo(ticker: string): Promise<TickerResolutionResult> {
   ...
   */
  async batchResolveTickers(
    tickers: string[], 
    options: TickerResolutionOptions = {}
  ): Promise<Record<string, TickerResolutionResult>> {
    const results: Record<string, TickerResolutionResult> = {};
