...
export class FilingStorage {
    constructor(options = {}) {
        ...
        // Default options
        this.defaultOptions = Object.assign({ deduplicateByUrl: true, updateExisting: true, archiveThresholdDays: 365, batchSize: 100 }, options.defaultOptions);
    }
    /**
    ...
    async storeFiling(filing, options = {}) {
        ...
        try {
            ...
            if (!tickerResolution.success) {
                throw new SECEdgarError(`Failed to resolve ticker ${filing.ticker}: ${tickerResolution.error}`, SECErrorCode.NOT_FOUND);
            }
            ...
            for (const ticker of tickers) {
                const summary = await this.prisma.summary.create({
                    data: {
                        ...
                        summaryJSON: Object.assign(Object.assign({}, filing), { 
            ...
            return Object.assign({}, filing);
        }
        catch (error) {
            console.error(`Error storing filing for ${filing.ticker} (${filing.filingType}):`, error);
            throw new SECEdgarError(`Failed to store filing: ${error.message}`, SECErrorCode.UNKNOWN_ERROR);
        }
    }
    /**
    ...
    async findFilingByUrl(url) {
        ...
        try {
            ...
            const filing = Object.assign({ id: summary.id, companyName: ticker.companyName, ticker: ticker.symbol, cik: ((_a = summary.summaryJSON) === null || _a === void 0 ? void 0 : _a.cik) || '', filingType: summary.filingType, filingDate: summary.filingDate, filingUrl: summary.filingUrl, url: summary.filingUrl, description: summary.summaryText }, summary.summaryJSON);
            return filing;
        }
        catch (error) {
    ...
    async updateFiling(id, filing) {
        ...
        try {
            ...
            if (!summaries.length) {
                throw new SECEdgarError(`Filing with ID ${id} not found`, SECErrorCode.NOT_FOUND);
            }
            // Update each summary
            for (const summary of summaries) {
                await this.prisma.summary.update({
                    ...
                    data: Object.assign({ filingType: filing.filingType || summary.filingType, filingDate: filing.filingDate || summary.filingDate, summaryText: filing.description || summary.summaryText }, (filing.content ? {
                        summaryJSON: Object.assign(Object.assign(Object.assign({}, summary.summaryJSON), filing), { content: filing.content.substring(0, 10000) })
                    } : {}))
                });
            }
            // Return the first summary as the updated filing
            const firstSummary = summaries[0];
            return Object.assign(Object.assign({ id: firstSummary.id, companyName: firstSummary.ticker.companyName, ticker: firstSummary.ticker.symbol, cik: ((_a = firstSummary.summaryJSON) === null || _a === void 0 ? void 0 : _a.cik) || '', filingType: (filing.filingType || firstSummary.filingType), filingDate: filing.filingDate || firstSummary.filingDate, filingUrl: firstSummary.filingUrl, url: firstSummary.filingUrl, description: filing.description || firstSummary.summaryText }, firstSummary.summaryJSON), filing);
        }
        catch (error) {
            ...
            throw new SECEdgarError(`Failed to update filing: ${error.message}`, SECErrorCode.UNKNOWN_ERROR);
        }
    }
    /**
    ...
    async storeFilingsBatch(filings, options = {}) {
        ...
        for (let i = 0; i < filings.length; i += batchSize) {
            ...
            try {
                ...
                throw new SECEdgarError(`Failed to store batch of filings: ${error.message}`, SECErrorCode.UNKNOWN_ERROR);
            }
        }
        return results;
    }
    ...
    async findFilingsByDateRange(options) {
        ...
        try {
            ...
            if (filingTypes === null || filingTypes === void 0 ? void 0 : filingTypes.length) {
            ...
            return summaries.map((summary) => {
                var _a;
                return (Object.assign({ id: summary.id, companyName: summary.ticker.companyName, ticker: summary.ticker.symbol, cik: ((_a = summary.summaryJSON) === null || _a === void 0 ? void 0 : _a.cik) || '', filingType: summary.filingType, filingDate: summary.filingDate, filingUrl: summary.filingUrl, url: summary.filingUrl, description: summary.summaryText }, summary.summaryJSON));
            });
        }
        catch (error) {
