...
// Eligibility gate combines three filters before the (expensive) x_search call
// fires:
...
//
// Forms set is inlined (rather than read from `form-registry.ts`) because the
// registry has a stale `.js` shadow that wins jest module resolution.
...
// Forms we recognize but treat as low-signal for sentiment. Used to
// distinguish "unknown form" from "known but not eligible" so the skip
// reason is precise.
...
//   - Grandfathered: BRK.A (S&P uses B-class only) and WBA (left the index
//     but remains a household-name mega-cap)
//
// Sorted alphabetically for diff readability. The index changes ~25 tickers
// per year on rebalancing; refresh this list opportunistically. A delisted
// name remaining briefly is harmless (eligibility falls through to
// `not_in_allowlist`). A missing new addition means a legitimate ticker
// doesn't get x_sentiment enrichment until the next refresh.
//
// Extend at runtime via `X_SENTIMENT_ALLOWLIST_EXTRA` env var.
...
function normalizeTicker(input: string | null | undefined): string {
  if (!input) return '';
...
function checkEligibility(ticker: string | null | undefined, formType: string | null | undefined): EligibilityResult {
  const normalized = normalizeTicker(ticker);
  if (!normalized) {
    return { eligible: false, ticker: normalized, reason: 'no_ticker' };
  }
  if (!TICKER_REGEX.test(normalized)) {
    return { eligible: false, ticker: normalized, reason: 'invalid_ticker_format' };
  }
  const form = String(formType ?? '').toUpperCase().trim();
  if (!form) {
    return { eligible: false, ticker: normalized, reason: 'unknown_form_type' };
  }
  if (HIGH_IMPORTANCE_FORMS.has(form)) {
    // fall through — eligible on form dimension
  } else if (KNOWN_LOW_IMPORTANCE_FORMS.has(form)) {
    return { eligible: false, ticker: normalized, reason: 'low_importance_form' };
  } else {
    return { eligible: false, ticker: normalized, reason: 'unknown_form_type' };
  }
  if (!getAllowlist().has(normalized)) {
    return { eligible: false, ticker: normalized, reason: 'not_in_allowlist' };
  }
  return { eligible: true, ticker: normalized };
}
...
export async function getXSentiment(input: XSentimentInput): Promise<XSentimentResult> {
  ...
  const eligibility = checkEligibility(input.ticker, input.formType);
...
/** Test-only: reset env-derived allowlist cache between tests. */
