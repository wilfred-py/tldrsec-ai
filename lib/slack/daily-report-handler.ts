...
// report. Previously lived in lib/ai/openrouter-credit-monitor.ts as a
// one-caller module; inlined here because the surface (fetch → format for
// this file's own Slack payloads) has no reuse potential and no non-Slack
// dependency. See docs/plans/2026-01-02-remove-budget-system-add-credit-monitoring.md.

const CREDIT_WARNING_THRESHOLD = parseInt(process.env.OPENROUTER_CREDIT_WARNING_THRESHOLD || '50', 10);
...
async function getOpenRouterCreditStatus(): Promise<CreditStatus> {
  ...
  if (!apiKey) {
    dailyReportLogger.error('No OpenRouter API key configured');
    return {
      credits: 0,
      limit: 0,
      usage: 0,
  ...
  try {
    ...
    if (!response.ok) {
      ...
      dailyReportLogger.error('Failed to fetch OpenRouter credit status', {
    ...
    dailyReportLogger.info('OpenRouter credit status retrieved', {
      credits,
      limit,
      usage,
      isLow,
      limitReached,
      warningThreshold: CREDIT_WARNING_THRESHOLD,
    ...
    dailyReportLogger.error('Exception fetching OpenRouter credit status', { error: errorMessage });
    return {
      credits: 0,
      limit: 0,
      usage: 0,
...
function formatCreditStatusForSlack(status: CreditStatus): {
  ...
  if (status.isLow) {
    return {
      text: `⚠️ *Low Credits Warning!* Remaining: $${status.credits.toFixed(2)} (below $${CREDIT_WARNING_THRESHOLD} threshold)`,
      emoji: '⚠️',
      color: '#ffc107'
    };
  }
