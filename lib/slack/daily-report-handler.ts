...
//
// This module's only production caller was this Slack daily-report handler.
// Its two functions — `getOpenRouterCreditStatus` (HTTP fetch) and
// `formatCreditStatusForSlack` (Slack-tone rendering) — both live exclusively
// for the Slack credit-status block rendered in `generateHourlySummary`.
// Both functions were private below the **interface** of "the hourly Slack
// report"; the seam was hypothetical (one **adapter**, one caller, zero
// tests). Inlined for **locality**.
//
