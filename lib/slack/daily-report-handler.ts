...
// =============================================================================
// OpenRouter Credit Status (private to the daily-report seam)
// =============================================================================
// The only production caller of the credit monitor is this handler's
// generateHourlySummary path. Kept private so the OpenRouter credit
// shape and the Slack rendering share one address.

const CREDIT_WARNING_THRESHOLD = parseInt(
...
async function getOpenRouterCreditStatus(): Promise<CreditStatus> {
  ...
  try {
    ...
    creditLogger.error('Exception fetching OpenRouter credit status', {
      error: errorMessage,
    });
    return {
      credits: 0,
