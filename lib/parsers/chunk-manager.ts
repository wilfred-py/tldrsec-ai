...
export interface ChunkOptions {
  ...
  // Whether to respect semantic boundaries (section, paragraphs, etc)
  respectSemanticBoundaries?: boolean;
  
  // Minimum size of a chunk in characters
  minChunkSize?: number;
  
  // Whether to include tables in chunks
  includeTables?: boolean;
  
  // Whether to include lists in chunks
  includeLists?: boolean;
  
  // Custom separator to use between chunks when reconstructing
  separator?: string;
}
...
export interface DocumentChunk {
  id: number;           // Unique ID for this chunk
  content: string;      // The text content of the chunk
  metadata: {
    start: number;      // Starting offset in the original document
    end: number;        // Ending offset in the original document
    sectionTypes: FilingSectionType[]; // Types of sections in this chunk
    sectionTitles: string[];  // Titles of sections in this chunk
    isTable?: boolean;  // Whether this chunk contains tables
    isList?: boolean;   // Whether this chunk contains lists
    charCount: number;  // Number of characters in the chunk
  };
}
 ...
 */
export function chunkParsedFiling(
  filing: ParsedSECFiling,
  ...
  try {
    ...
    return sizeBasedChunking(
      filing.fullText || JSON.stringify(filing.importantSections),
      mergedOptions
    );
  } catch (error) {
...
function semanticChunking(
  ...
  let currentChunkMetadata = {
    sectionTypes: [] as FilingSectionType[],
    sectionTitles: [] as string[],
    isTable: false,
    isList: false,
  };
  
  const processSection = (section: FilingSection, depth = 0) => {
  ...
  return {
    ...
    metadata: {
      ...
      documentTitle: filing.importantSections['Title'] || filing.metadata?.companyName
    }
  };
}
...
function findChunkBoundaries(
  ...
  const boundaries: number[] = [0]; // Start with the beginning of text
 ...
 */
export function measureMemoryUsage(input: ParsedSECFiling | string): number {
  let byteSize = 0;
