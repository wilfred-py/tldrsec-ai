/**
 * Test script for enhanced SEC filing extraction and Claude AI summarization
 * 
 * This script demonstrates how the improved SEC filing extractor can be integrated
 * with the Claude AI summarization pipeline to produce better summaries.
 */
...
// Import the SEC filing integration module
import { 
...
const OUTPUT_DIR = path.join(__dirname, 'enhanced-summarization-test-results');
...
async function runTests() {
  console.log('Starting enhanced SEC filing extraction and summarization tests...');
  ...
  for (let i = 0; i < TEST_URLS.length; i++) {
    ...
    try {
      ...
      console.log('Generating prompt for Claude AI...');
      const filingType = extractionResult.metadata?.filingType || '10-K';
      const prompt = generateSummaryPrompt(formattedContent, filingType, 'Tesla, Inc.');
      
      // Save the prompt
      writeTestResult(`claude_prompt_${i + 1}.txt`, prompt);
      
      // Call Claude AI to generate summary
      console.log('Calling Claude AI to generate summary...');
...
// Run the tests
console.log('=== Enhanced SEC Filing Extraction and Summarization Test ===');
runTests()
  .then(() => console.log('\nTests completed. Check the enhanced-summarization-test-results directory for output.'))
  .catch(error => {
